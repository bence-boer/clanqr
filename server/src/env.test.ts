...
describe('env schema', () => {
    ...
    it('fails when SUPABASE_KEY is missing', () => {
        const without_key: Record<string, string | number> = { ...VALID_ENV };
        delete (without_key as Record<string, unknown>)['SUPABASE_KEY'];
    ...
    it('applies defaults when optional vars are missing', () => {
        ...
        if (result.success) {
            ...
            expect(result.data.GITHUB_CALLBACK_URL).toBe('http://localhost:3001/api/auth/login/callback');
            expect(result.data.ADMIN_GITHUB_IDS).toBe('');
            expect(result.data.PORT).toBe(3001);
