...
export async function get_usage_summary(supabase: TypedSupabaseClient): Promise<UsageSummaryStats> {
    ...
    const { data: stats_data } = await supabase
        .from('agent_sessions')
        .select('duration_ms, prompt_tokens, completion_tokens, cache_read_tokens, cache_write_tokens, status');
...
export async function get_usage_breakdown(supabase: TypedSupabaseClient): Promise<UsageBreakdownStats> {
    const { data: runs, error } = await supabase
        .from('agent_sessions')
        .select('agent_type, model, status, prompt_tokens, completion_tokens, cache_read_tokens, cache_write_tokens');
