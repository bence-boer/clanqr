...
describe("PipelineService", () => {
    beforeEach(() => {
        ...
        (pipeline_service as any).state = "idle";
        (pipeline_service as any).active_run = null;
        (pipeline_service as any).is_processing = false;
    ...
    describe("get_status", () => {
        ...
        it("reflects paused state", () => {
            (pipeline_service as any).state = "paused";
            expect(pipeline_service.get_status().state).toBe("paused");
        });

        it("reflects active run info", () => {
            (pipeline_service as any).state = "running";
            (pipeline_service as any).active_run = {
                task_id: "task-1",
                run_id: "run-1",
                feature_id: "feat-1",
            };
    ...
    describe("process_next", () => {
        ...
        it("returns immediately when already processing", async () => {
            (pipeline_service as any).is_processing = true;
            await pipeline_service.process_next();
        ...
        it("returns immediately when active_run exists", async () => {
            (pipeline_service as any).active_run = { task_id: "x", run_id: "y", feature_id: "z" };
            await pipeline_service.process_next();
            expect(pipeline_service.get_status().state).toBe("idle");
        });
        ...
        it("does not exceed concurrency limit", async () => {
            ...
            expect((pipeline_service as any).active_run).toBeNull();
        });
    });
    ...
    describe("get_log", () => {
        ...
        it("delegates to agent_service when active run exists", () => {
            (pipeline_service as any).active_run = { task_id: "task-1", run_id: "run-1", feature_id: "feat-1" };
