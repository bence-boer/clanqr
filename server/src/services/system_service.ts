...
export type { SystemAlert } from './system_alerts';
export { check_system_alerts } from './system_alerts';
export { get_models } from './model_service';
...
function read_memory(): Pick<SystemStats, 'memory_total_mb' | 'memory_used_mb' | 'memory_percent'> {
    try {
...
async function read_storage(): Promise<Pick<SystemStats, 'storage_total_gb' | 'storage_used_gb' | 'storage_percent'>> {
    try {
        const proc = Bun.spawn(['df', '-BG', '/'], { stdout: 'pipe', stderr: 'pipe' });
        await proc.exited;
        ...
        return {
            ...
            storage_percent: total_gb > 0 ? Math.round((used_gb / total_gb) * 100) : 0
        };
    }
    catch (error) {
