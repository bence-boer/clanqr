...
export function create_feature_handlers(get_feature: () => Feature, on_update: () => Promise<void>) {
    return {
        ...
        update_task: (task_id: string, description: string, model?: string | null) => safe_call(() => api.update_task(task_id, { description, ...(model !== undefined ? { model } : {}) }), 'update task', on_update),
        ...
        save_edit: async (data: { title: string, description?: string | null, cli: string, planning_model: string | null, execution_model: string | null }) => {
