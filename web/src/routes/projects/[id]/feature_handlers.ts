...
export function create_feature_handlers(get_feature: () => Feature, on_update: () => Promise<void>) {
    return {
        ...
        update_task: (task_id: string, description: string) => safe_call(() => api.update_task(task_id, { description }), 'update task', on_update),
        ...
        save_edit: async (data: { title: string, description?: string | null, cli: string, model: string | null }) => {
