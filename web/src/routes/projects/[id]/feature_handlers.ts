...
export function create_feature_handlers(get_feature: () => Feature, on_update: () => Promise<void>) {
    return {
        ...
        update_task: (task_id: string, description: string, model?: string | null, title?: string | null) => safe_call(() => api.update_task(task_id, { description, title, ...(model !== undefined ? { model } : {}) }), 'update task', on_update),
