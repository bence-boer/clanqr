<script lang="ts">
  ...
  async function create_feature(data: { title: string; description?: string; cli: string; model: string | null; on_task_failure: FailureBehavior; task_timeout_minutes: number; resources: { url: string; title?: string }[] }) {
    try {
  ...
  {:else}
    ...
    <div class="content-grid" class:show-detail={show_mobile_detail}>
      <FeatureList
        ...
        on_delete_selected={delete_selected_features}
      />
