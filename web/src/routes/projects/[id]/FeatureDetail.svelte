<script lang="ts">
    ...
    $effect(() => {
        if (editing) load_edit_models(edit_cli);
    });
...
<div class="detail-body">
    {#if editing}
        <form class="edit-feature-form" onsubmit={(e) => {
            e.preventDefault();
            save_edit();
        }}>
    ...
    {:else}
        ...
        <ResourceList
            {feature}
            on_add_resource={handlers.add_resource}
            on_remove_resource={handlers.remove_resource}
        />
    <TaskList
        {feature}
        on_approve={handlers.approve}
        on_approve_all={handlers.approve_all}
        on_spawn={handlers.spawn}
        on_add={handlers.add_task}
        on_update={handlers.update_task}
        on_delete={handlers.delete_task}
